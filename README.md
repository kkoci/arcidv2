# ArcID — Licensed AI Training Compensation Rail

> **Pay independent artists automatically when AI trains on their music — provably, without exposing the training corpus.** An AI company escrows USDC and commits a Merkle root over its training corpus before training. A TEE-attested enclave verifies the corpus against that commitment and against artist licensing, computes each artist's share, and signs it. Artists claim their exact payout on-chain with a real Merkle proof.

Concretely: an artist registers a track (fingerprint hash + rights commitment). An AI company deposits USDC into a pool and commits `corpusRoot = MerkleRoot(fingerprintHash[])` before training. A real TEE enclave ingests the claimed corpus, checks it against that commitment and against the artist registry, computes an equal-split allocation, and submits it on-chain. Each artist claims their share with a Merkle proof verified by the real, deployed `MerkleProof.sol`. **Live-verified end to end on Arc testnet — real contracts, a real TEE-registered enclave wallet, real USDC landing in real artist wallets, real transaction hashes.** See [CHANGELOG.md](CHANGELOG.md)'s "Licensed AI Training Compensation Rail" entry.

This rail is built on arcid2's underlying bonded-attestation protocol — post collateral, let a TEE-attested party judge or compute an outcome, settle automatically on-chain, no human review. That same protocol also runs two earlier, still-live verticals: a **Data SLA Bond** (an oracle sells price data; breach verification needs an LLM's judgment) and a **Proof-of-Exploit Bounty** (a bug-bounty payout decided by literally running the attack, deterministic, no LLM anywhere in the payout path). Both are still deployed and still working on Arc testnet — evidence the underlying protocol generalizes, not co-equal products on this page. See the [Proof-of-Exploit section](#proof-of-exploit--a-second-vertical-on-the-same-primitive) below, and CHANGELOG.md for all three.

Arc's own agent-identity standard, **ERC-8004**, explicitly excludes bonds and slashing from its scope ("incentives and slashing... are outside the scope of this registry") — the underlying protocol arcid2 (and this rail) is built on is an early working implementation of the stake-secured validator pattern ERC-8004 already anticipates.
