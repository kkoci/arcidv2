...
- **Agent Marketplace listing (prepared, not yet submitted):** `MARKETPLACE_LISTING.md` — a complete submission package in Circle's real listing schema (reverse-engineered from live `circle services search` results), framed around the trust-layer pitch rather than just "price feed." Circle's listing intake is a Google Form, not a self-serve API — mirrors how the USYC allowlist gap above is handled: the artifact is ready and verified, the last external step (submit the form, once the oracle is redeployed and publicly reachable again) is called out honestly rather than glossed over.
